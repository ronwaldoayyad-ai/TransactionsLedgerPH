...
// Data layer for the Loan Disbursement feature. RLS scopes reads: admins get
// every disbursement, borrowers get only their own ASSIGNED ones. Admin writes
// flow through the create_loan_disbursement RPC (atomic unique numbering) +
// direct assign/status/delete; the borrower's acknowledgment flows through the
// acknowledge_loan_disbursement RPC. Realtime keeps both sides in sync.
const DisbursementsContext = createContext(null)

const mapDisbursement = (r) => ({
  id: r.id,
...
// Demo store: module-scoped so it survives navigation within the tab. Seeds one
// ASSIGNED, not-yet-acknowledged disbursement for the mock borrower Maria
// (u-001); it is paired with the "Loan Disbursement Ready" seed notification so
// the whole generate → notify → accept loop is clickable on localhost without
// the backend. Objects are stored in the final camelCase shape (no mapping).
...
export function DisbursementsProvider({ children }) {
  ...
  const [liveDisbursements, setLiveDisbursements] = useState([])
  const [demoVersion, setDemoVersion] = useState(0)
  const [loading, setLoading] = useState(isLive)

  // Admin sees all; a borrower sees only their own. Live rows are already
  // RLS-scoped; the demo store is scoped here to mirror that.
  ...
  // Admin: generate a draft disbursement (unique number assigned server-side).
  ...
  // Admin: assign a draft to the borrower (makes it visible to them). The
  // PDF-carrying "Loan Disbursement Ready" notification is created by the caller
  // (client-side) because the PDF is rendered with jsPDF.
  ...
  // Borrower: acknowledge (accept) their own assigned disbursement. The RPC
  // stamps acceptance and notifies every admin (SECURITY DEFINER).
