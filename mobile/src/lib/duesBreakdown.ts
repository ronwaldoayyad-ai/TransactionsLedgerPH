...
export type DuesSegmentKey = 'paid' | 'upcoming' | 'due' | 'past_due'

export type DuesSegment = {
  key: DuesSegmentKey
  label: string
  color: string
  amount: number
...
export type NextPayment = {
  ...
  kind: 'past_due' | 'due' | 'upcoming'
}

export type Payoff = {
  paidCount: number
  totalCount: number
  pct: number // 0..100, share of installments paid
  payoffDate: string | null // last scheduled due date
}

export type DuesBreakdown = {
  segments: DuesSegment[] // always 4, fixed order (zero-value segments included)
  totalAmount: number
  totalCount: number
  paidAmount: number
  paidCount: number
  ...
  paidPctAmount: number // 0..100
  paidPctCount: number // 0..100
  isEmpty: boolean // no installment transactions at all
  allSettled: boolean // has installments and nothing left to pay
  nextPayment: NextPayment | null
  streak: number // consecutive on-time paid installments (most recent backwards)
  payoff: Payoff
}

// Fixed segment order + palette (mirrors theme.ts / ProgressBar emerald).
const SEGMENT_META: { key: DuesSegmentKey; label: string; color: string }[] = [
...
function segmentFor(t: any, today: string): DuesSegmentKey | null {
...
export function buildDuesBreakdown(
  transactions: any[],
  today: string = toISODate(new Date()),
  loanId?: string,
): DuesBreakdown {
  const items = transactions.filter(
    (t: any) => t.type === 'Installment' && (!loanId || t.loanId === loanId),
  )

  const totals: Record<DuesSegmentKey, { amount: number; count: number }> = {
  ...
  const segments: DuesSegment[] = SEGMENT_META.map((m) => ({
    ...m,
  ...
  // Next payment: the earliest still-owed due date, aggregated across any
  // items sharing it. Past due dates sort first, so the most urgent surfaces.
  const owed = items.filter((t: any) => {
    const s = effectiveStatus(t, today)
    return s === 'unpaid' || s === 'past_due'
  })
  let nextPayment: NextPayment | null = null
  if (owed.length > 0) {
    const dueDate = owed.reduce(
      (min: string, t: any) => (t.dueDate < min ? t.dueDate : min),
      owed[0].dueDate,
    )
    const onDate = owed.filter((t: any) => t.dueDate === dueDate)
    const daysUntil = daysBetween(today, dueDate)
    nextPayment = {
      dueDate,
      amount: onDate.reduce((s: number, t: any) => s + t.amount, 0),
      count: onDate.length,
      daysUntil,
  ...
  const paidItems = items
    .filter((t: any) => effectiveStatus(t, today) === 'paid')
  ...
  const payoffDate =
    items.length > 0
      ? items.reduce((max: string, t: any) => (t.dueDate > max ? t.dueDate : max), items[0].dueDate)
      : null
