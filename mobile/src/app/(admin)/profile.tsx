...
// Admin self-service profile (web profile modal, admin variant): name / nickname
// / phone / email edits via update_my_profile, avatar upload, sign out.
export default function AdminProfile() {
  ...
  const save = async () => {
    setSaving(true)
    const { error } = await updateMyProfile({ firstName, lastName, nickname, phone, email })
    setSaving(false)
    if (error) {
      errorHaptic()
      ...
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerClassName="gap-4 p-4 pb-10" keyboardShouldPersistTaps="handled">
          <FadeInView className="items-center py-4">
            ...
            <Text className="font-sans text-xs text-slate-500">Administrator</Text>
          </FadeInView>
          ...
          <FadeInView delay={80} className="gap-4 rounded-2xl border border-slate-200/70 bg-white p-5 shadow-sm">
            ...
            <FloatingInput label="Nickname (shown around the app)" value={nickname} onChangeText={setNickname} />
